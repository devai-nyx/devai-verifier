...
function validateOutputContract(contract, label) {
  ...
  if (contract.paths.length === 0) {
    throw new VerificationError(
      "SCHEMA_INVALID",
      `${label}.paths must be nonempty when present`,
    );
  }
  for (const path of contract.paths)
    assertString(path, `${label} path`, PORTABLE_PATH);
...
function validateTaskPolicy(policy) {
  assertExactKeys(
    policy,
    ["repositoryId", "requiredNodes", "schemaVersion"],
    "task policy",
  );
  if (policy.schemaVersion !== "1.0.0" && policy.schemaVersion !== "1.1.0") {
    throw new VerificationError(
      "SCHEMA_INVALID",
      "unsupported task-policy schemaVersion",
    );
  }
  assertString(policy.repositoryId, "task policy repositoryId", IDENTIFIER);
  if (
    !Array.isArray(policy.requiredNodes) ||
    policy.requiredNodes.length === 0
  ) {
    throw new VerificationError(
      "SCHEMA_INVALID",
      "task policy requiredNodes must be nonempty",
    );
  }
  const nodeIds = [];
  ...
  const visit = (nodeId) => {
    if (visiting.has(nodeId)) {
      throw new VerificationError(
        "SCHEMA_INVALID",
        `task policy contains a cycle at ${nodeId}`,
      );
    }
...
function validateTrustStore(trust) {
  ...
  if (trust.schemaVersion !== "1.0.0") {
    throw new VerificationError(
      "SCHEMA_INVALID",
      "unsupported trust-store schemaVersion",
    );
  }
  if (
    !Array.isArray(trust.trustedSigners) ||
    trust.trustedSigners.length === 0
  ) {
    throw new VerificationError(
      "SCHEMA_INVALID",
      "trustedSigners must be nonempty",
    );
  }
  const signerIds = [];
...
function validateEnvelope(envelope) {
  assertExactKeys(
    envelope,
    ["payload", "payloadType", "schemaVersion", "signatures"],
    "envelope",
  );
  if (
  ...
  if (!Array.isArray(envelope.signatures) || envelope.signatures.length !== 1) {
    throw new VerificationError(
      "SCHEMA_INVALID",
      "envelope must have exactly one signature",
    );
  }
  assertExactKeys(
  ...
  assertString(
    envelope.signatures[0].signerId,
    "envelope signerId",
    IDENTIFIER,
  );
  assertString(
...
function validateReceipt(receipt) {
  ...
  if (receipt.schemaVersion !== "1.0.0" && receipt.schemaVersion !== "1.1.0") {
    throw new VerificationError(
      "SCHEMA_INVALID",
      "unsupported candidate-receipt schemaVersion",
    );
  }
  assertExactKeys(
    receipt.repository,
    ["commit", "id", "tree"],
    "candidate receipt repository",
  );
  assertString(
    receipt.repository.id,
    "candidate receipt repository ID",
    IDENTIFIER,
  );
  assertString(
  ...
  assertString(
    receipt.taskPolicyDigest,
    "candidate receipt taskPolicyDigest",
    SHA256,
  );
  if (receipt.profile !== "affected" && receipt.profile !== "rc") {
    throw new VerificationError(
      "SCHEMA_INVALID",
      "candidate receipt profile is invalid",
    );
  }
  ...
  if (!Array.isArray(receipt.tasks)) {
    throw new VerificationError(
      "SCHEMA_INVALID",
      "candidate receipt tasks must be an array",
    );
  }
  const ids = [];
...
function validateTaskResult(result, label) {
  ...
  assertObject(
    result.dependencyResultDigests,
    `${label}.dependencyResultDigests`,
  );
  ...
  if (result.status !== "PASS") {
    const rendered =
      typeof result.status === "string" ? result.status : "<malformed>";
    throw new VerificationError(
      "TASK_STATUS_NOT_PASS",
      `${label} status is ${rendered}, not PASS`,
    );
  }
}
...
function filesBelow(root, current = root) {
  ...
  for (const entry of readdirSync(current, { withFileTypes: true })) {
    ...
    if (entry.isSymbolicLink()) {
      throw new VerificationError(
        "ARTIFACT_SYMLINK",
        "artifact bundle contains a symbolic link",
      );
    }
...
function verifyArtifacts(policy, results, artifactsDir, candidate) {
  ...
  const policyById = new Map(
    policy.requiredNodes.map((node) => [node.nodeId, node]),
  );
  for (const [nodeId, result] of results) {
    ...
    if (
      ...
      actualOutputNames.some(
        (name, index) => name !== expectedOutputNames[index],
      )
    ) {
      throw new VerificationError(
    ...
    for (const path of paths) {
      ...
      if (fromRoot.startsWith(`..${sep}`) || fromRoot === "..") {
        throw new VerificationError(
          ...
          `artifact ${path} escapes its bundle`,
        );
      }
      let stat;
      try {
...
export function verifyCandidateEvidence({
  ...
  if (taskPolicy.repositoryId !== expectedRepository) {
    throw new VerificationError(
      "REPOSITORY_MISMATCH",
      "task policy belongs to another repository",
    );
  }
  ...
  if (trustStore.revokedSignerIds.includes(signature.signerId)) {
    throw new VerificationError(
      "SIGNER_REVOKED",
      `signer ${signature.signerId} is revoked`,
    );
  }
  const signer = trustStore.trustedSigners.find(
    (entry) => entry.signerId === signature.signerId,
  );
  if (signer === undefined) {
    throw new VerificationError(
      "SIGNER_UNTRUSTED",
      `signer ${signature.signerId} is not trusted`,
    );
  }
  ...
  if (
    ...
    throw new VerificationError(
      "SIGNATURE_INVALID",
      "candidate receipt signature is invalid",
    );
  }
  ...
  if (
    bindingMode === "exact-commit" &&
    receipt.repository.commit !== expectedCommit
  ) {
    throw new VerificationError(
      "COMMIT_MISMATCH",
      "candidate receipt commit does not match",
    );
  }
  if (receipt.repository.tree !== expectedTree) {
    throw new VerificationError(
      "TREE_MISMATCH",
      "candidate receipt tree does not match",
    );
  }
  ...
  const expectedById = new Map(
    taskPolicy.requiredNodes.map((node) => [node.nodeId, node]),
  );
  ...
  for (const nodeId of expectedIds) {
    ...
    if (task.taskKey !== expected.taskKey) {
      throw new VerificationError(
        "TASK_KEY_STALE",
        `node ${nodeId} has a stale task key`,
      );
    }
    ...
    if (result.nodeId !== nodeId || result.taskKey !== expected.taskKey) {
      throw new VerificationError(
        "TASK_KEY_STALE",
        `task result ${nodeId} is not current`,
      );
    }
  ...
  for (const nodeId of expectedIds) {
    ...
    for (const dependency of expectedDependencies) {
      if (
        result.dependencyResultDigests[dependency] !==
        receiptById.get(dependency).resultDigest
      ) {
        throw new VerificationError(
          "DEPENDENCY_MISMATCH",
