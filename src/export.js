...
function outsideRepository(repo, path, label) {
  ...
  try {
    ...
    throw new VerificationError(
      "REPOSITORY_INVALID",
      "candidate repository is unreadable",
    );
  }
...
function outputDestination(repo, path) {
  ...
  try {
    ...
    throw new VerificationError(
      ...
      "evidence output parent is unavailable",
    );
  }
...
function readCommittedDescriptor(repo, commit) {
  ...
  try {
    text = execFileSync(
      "git",
      ["-C", repo, "show", `${commit}:test-tasks.json`],
      {
        encoding: "utf8",
...
function exactCandidate(repo, commit, tree) {
  if (!GIT_OBJECT.test(commit) || !GIT_OBJECT.test(tree)) {
    throw new VerificationError(
      "SCHEMA_INVALID",
      "exact commit and tree are required",
    );
  }
  ...
  if (git(repo, ["rev-parse", `${commit}^{tree}`]) !== tree) {
    throw new VerificationError(
      "TREE_MISMATCH",
      "candidate tree does not match requested tree",
    );
  }
}
...
function matchingKeyPair(privateKeyPath, publicKeyPath) {
  ...
  if (
    ...
    throw new VerificationError(
      "KEY_INVALID",
      "signing key pair must use Ed25519",
    );
  }
  ...
  if (!derived.equals(supplied)) {
    throw new VerificationError(
      "KEY_MISMATCH",
      "private and public key do not form a pair",
    );
  }
...
export function preflightCandidateEvidence(options) {
  ...
  const controlledToolchain = controlledInput(
    repository,
  ...
  const controlledPrivateKey = controlledInput(
    repository,
    options.privateKeyPath,
    "private key",
  );
  const controlledPublicKey = controlledInput(
    repository,
...
export function exportCandidateEvidence(options) {
  ...
  try {
    ...
    writeFileSync(
      join(staging, "envelope.json"),
      `${canonicalize(envelope)}\n`,
      { flag: "wx" },
    );
    writeFileSync(
      join(staging, "task-policy.json"),
      `${canonicalize(built.taskPolicy)}\n`,
      {
        flag: "wx",
      },
    );
    if (built.taskPolicy.schemaVersion === "1.0.0") {
      writeFileSync(
        join(staging, "trust-store.json"),
        `${canonicalize(trustStore)}\n`,
        { flag: "wx" },
      );
    }
    ...
    const manifest = {
      schemaVersion:
        built.taskPolicy.schemaVersion === "1.1.0" ? "1.1.0" : "1.0.0",
