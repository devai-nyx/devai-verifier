...
function parse(argv) {
  ...
  for (let index = 0; index < argv.length; index += 2) {
    ...
    if (
      (!required.has(name) && !optional.has(name)) ||
      values[name] !== undefined
    ) {
      throw new VerificationError(
        "USAGE",
        `unknown or duplicate argument --${name}`,
      );
    }
    values[name] = value;
  }
...
try {
  ...
  if (values.preflight !== undefined && values.preflight !== "true") {
    throw new VerificationError(
      "USAGE",
      "--preflight must be true when supplied",
    );
  }
  const options = {
    repo: values.repo,
  ...
  const result =
    values.preflight === "true"
      ? {
          ...
          preflight: true,
          taskPolicyDigest:
            preflightCandidateEvidence(options).built.taskPolicyDigest,
        }
